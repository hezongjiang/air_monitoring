    ...
    methods: {
        ...
        this.$axios
          ...
          .then(maah => {
            if (maah.data.successful) {
              ...
              console.log(this.optionAir.series)
      ...
      exportExcel(e) { // 导出excel文件
        ...
        if (this.typeState === 1) {
          ...
          fileName = this.beginEndTState[0] + '至' + this.beginEndTState[1] + this.airChooseState + '南庄一期原始数据曲线'
        } else if (this.typeState === 2) {
          data = this.tbList2.map(v => filterVal.map(k => v[k]))
          fileName = this.beginEndTState[0] + '至' + this.beginEndTState[1] + this.airChooseState + '南庄二期原始数据曲线'
        }
