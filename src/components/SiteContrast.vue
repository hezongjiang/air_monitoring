    ...
    methods: {
      ...
      exportExcel(e) { // 导出excel文件
        ...
        if (this.typeState === 1) {
          ...
          fileName = this.beginEndTState[0] + '至' + this.beginEndTState[1] + this.airChooseState + '南庄一期站点曲线对比'
        } else if (this.typeState === 2) {
          data = this.tbList2.map(v => filterVal.map(k => v[k]))
          fileName = this.beginEndTState[0] + '至' + this.beginEndTState[1] + this.airChooseState + '南庄二期站点曲线对比'
        }
        let [fileType, sheetName] = ['xlsx', '站点曲线对比']
